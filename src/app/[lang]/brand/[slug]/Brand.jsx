...
const Brand = ({ lang }) => {
    ...
    return (
        <div className="bg-[#F1F4F8]">
            ...
            <div className="relative flex flex-row md:flex-row max-w-screen-lg sm:mt-5 md:mt-5+6 lg:mt-5 w-full lg:mx-auto sm:mx-auto md:mx-auto mx-auto mt-10 items-center">
                <Breadcrumb breadcrumbs={breadcrumbs} lang={langValue} lastVal={data?.name} />
            </div>

            <div className="relative flex flex-col md:flex-row max-w-screen-lg w-full mt-5 mx-auto items-center">
                <h2 className="text-left text-xl font-bold">{data?.name}</h2>
                {/* params.slug.split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()).join(' ') */}
            </div>
